...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="""Examples:
  python summarize_script.py
  python summarize_script.py --exclude-dirs node_modules build dist .git venv
  python summarize_script.py --exclude-files secrets.txt config.yaml *.log
  python summarize_script.py --exclude-dirs ./build --exclude-files ./src/ignore_this.py
"""
    )

    parser.add_argument(
        "--exclude-dirs",
        nargs='+',  # Allows specifying multiple directories
        metavar='DIR',
        help="Space-separated list of directory paths to exclude (e.g., node_modules .git build)."
    )
    parser.add_argument(
        "--exclude-files",
        nargs='+',  # Allows specifying multiple files/patterns
        metavar='FILE_OR_PATTERN',
        help="Space-separated list of file names or paths to exclude (e.g., secrets.txt *.log temp.data)."
             # Note: For pattern matching like *.log, uncomment fnmatch lines in the code.
    )
